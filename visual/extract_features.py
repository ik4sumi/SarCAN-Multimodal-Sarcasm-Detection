...
def save_i3d_features():
    ...
    with h5py.File(SarcasmDataset.features_file_path('i3d', 'avg_pool_all'), 'w') as avg_pool_features_file:
        for video_id in dataset.video_ids:
            video_frame_count = dataset.frame_count_by_video_id[video_id]
            print(video_frame_count)
            feature_count = video_frame_count - 10 + 1
            #print(video_id)
            #print(feature_count)
        ...
        for instance in tqdm(torch.utils.data.DataLoader(dataset), desc="Extracting I3D features"):
            ...
            for i in range(feature_count):
                output = i3d.extract_features(frames[:, :, i:i + 10, :, :]).squeeze()
